...
public class Base implements IModel {
    ...
    public static <T> T find( int id, String canonicalClassName ) {
        try {
            ...
            Map<String, Object> parentDatas =
                    BaseHelper.getSuperDatas( (Integer) HM.get( "id" ), Class.forName( canonicalClassName ) );
    ...
    public <T extends Base> List<T> queryAll( String sqlQuery ) {
        ...
        List<T> arrayList = new ArrayList<T>( list.size() );
        try {
            Constructor<T> cons = (Constructor<T>) this.getClass().getConstructor( Map.class );
            for ( Map<String, Object> attr : list ) {
                arrayList.add( cons.newInstance( attr ) );
            }
        } catch ( Exception e ) {
            logger.error( "Error while querying for all where " + sqlQuery );
        }
    ...
    private static <T extends Base> boolean updateRelation( Class<? extends Base> classObject, T instanceObject ) {
        ...
        for ( Field field : fields ) {
            ...
            if ( annotations.length > 0 ) {
                ...
                if ( annotationName.contentEquals( "ActiveRelationManyToMany" ) ||
                        annotationName.contentEquals( "ActiveRelationHasMany" ) ) {
    ...
    private static <T extends Base> void updateManyRelation( Field field, Annotation annotation, Class<T> classObject,
        ...
        String relationTable = relationName.contentEquals( "ActiveRelationManyToMany" ) ?
                ((ActiveRelationManyToMany) annotation).relationTable() :
                BaseHelper.getClassTableName( relationSimpleName );
        ...
        while ( savedCollection.hasNext() ) {
            ...
            String idFieldName = relationName.contentEquals( "ActiveRelationManyToMany" ) ?
                    StringUtils.uncapitalize( relationSimpleName ) + "Id" : "id";
        ...
        try {
            for ( int i = 0; i < collection.size(); i++ ) {
                ...
                //Already saved Object but is new to this object's collection save according to activerelation
                    ...
                    switch ( relationName.toString() ) {
                        case "ActiveRelationManyToMany":
                            existingObjectManyToMany( id, collectionObjectId, relationSimpleName, relationTable,
                                    classObject );
                            break;
    ...
    private static void newObjectManyToMany( Integer id, Object collectionObject, String relationCanonicalName,
            String relationSimpleName, String relationTable, Class<?> classObject ) {
    ...
    private static void existingObjectManyToMany( Integer id, Integer collectionObjectId, String relationSimpleName,
            String relationTable, Class<?> classObject ) {
    ...
    private static void newObjectHasMany( Integer id, Object collectionObject, String relationCanonicalName,
            Class<?> classObject ) {
        ...
        try {
            ...
            // id
            Method setRelationIdMethod =
                    Class.forName( relationCanonicalName ).getDeclaredMethod( setterMethod, Integer.class );
    ...
    private static void existingObjectHasMany( Integer id, Object collectionObject, String relationCanonicalName,
            Class<?> classObject ) {
                    ...
                    try {
                        ...
                        for ( int i = 0; i < manyToManyCollection.size(); i++ ) {
                            ...
                            if ( collectionObjectId == null ) {
                                ...
                                Map<String, Object> conditions = new HashMap<String, Object>();
                                conditions.put( StringUtils.uncapitalize( classObject.getSimpleName() ) + "Id", id );
                                conditions.put( StringUtils.uncapitalize( relationSimpleName ) + "Id",
                                        newCollectionId );
                                Manager.save( relationTable, conditions );
    ...
    public void initRelation( String attributeName ) {
        ...
        try {
            ...
            if ( !relationLoaded( relation ) ) {
                if ( this.id == null ) {
                    try {
                        if ( !relationAnnotations[0].annotationType()
                                .getSimpleName()
                                .contentEquals( "ActiveRelationHasOne" ) ) {
                ...
                switch ( relationAnnotations[0].annotationType().getSimpleName() ) {
                    ...
                    case "ActiveRelationManyToMany":
                        try {
                            relationHasMany( relation,
                                    ((ActiveRelationManyToMany) relationAnnotations[0]).relationTable() );
        ...
        } catch ( NoSuchFieldException | SecurityException | NegativeArraySizeException | IllegalAccessException e ) {
            logger.error( "Error while trying to initialize relation for " + attributeName, e );
        }
    }

    private boolean relationLoaded( Field relation ) throws IllegalAccessException {
        return relation.get( this ) != null;
    }
    ...
    private void relationHasOne( Field relation ) {
        ...
        try {
            Field relationIdField =
                    this.getClass().getDeclaredField( StringUtils.uncapitalize( relationSimpleName ) + "Id" );
            relationIdField.setAccessible( true );
            Method findMethod =
                    Class.forName( relationCanonicalClassName ).getMethod( "find", int.class, String.class );
    ...
    private void relationHasMany( Field relation, String relationTableName ) {
        ...
        relationTableName =
                relationTableName == null ? BaseHelper.getClassTableName( relationSimpleClassName ) : relationTableName;
        ...
        for ( int i = 0; i < size; i++ ) {
            try {
                if ( !isManyToMany ) {
                    ...
                    Object a = classObjectOfRelation.getMethod( "find", int.class )
                            .invoke( classObjectOfRelation.getConstructor().newInstance(), (int) results.get( i )
                                    .get( StringUtils.uncapitalize( relationSimpleClassName ) + "Id" ) );
                    collection.add( a );
                }
